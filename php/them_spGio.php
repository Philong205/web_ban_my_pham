...
//         // Trừ tồn kho và ẩn sản phẩm nếu hết
//         $tonKhoMoi = $tonKho - $newQuantity;
//         $db->update("sanpham", ["SoLuong" => $tonKhoMoi], "MaSP = $productId");
//         if ($tonKhoMoi <= 0) {
//             $db->update("sanpham", ["TrangThai" => 0], "MaSP = $productId");
//         }
...
try {
    ...
    if ($action === "update") {
        ...
        // Trừ tồn kho và ẩn sản phẩm nếu hết
        $tonKhoMoi = $tonKho - $quantity; // Chỉ trừ số lượng mới thêm
        $db->update("sanpham", ["SoLuong" => $tonKhoMoi], "MaSP = $productId");
        if ($tonKhoMoi <= 0) {
            $db->update("sanpham", ["TrangThai" => 0], "MaSP = $productId");
        }
